...
#include "scsd-sharing-manager.h"
...
#define RYGEL_BUS_NAME "io.github.scarecrow_de.Rygel1"
#define RYGEL_OBJECT_PATH "/io/github/scarecrow_de/Rygel1"
...
scsd_sharing_manager_init (GsdSharingManager *manager)
{
        ...
        for (i = 0; i < G_N_ELEMENTS (services); i++) {
                ...
                path = g_strdup_printf ("/io/github/scarecrow_de/settings-daemon/plugins/sharing/%s/", services[i]);
                service->settings = g_settings_new_with_path ("io.github.scarecrow_de.settings-daemon.plugins.sharing.service", path);
                g_free (path);
