...
#define GSD_KEYBOARD_DIR "io.github.scarecrow_de.settings-daemon.peripherals.keyboard"
...
convert_libgnomekbd_options (GSettings *settings)
{
        ...
        if (!schema_is_installed ("io.github.scarecrow_de.libgnomekbd.keyboard"))
                return;

        opt_array = g_ptr_array_new_with_free_func (g_free);

        libgnomekbd_settings = g_settings_new ("io.github.scarecrow_de.libgnomekbd.keyboard");
